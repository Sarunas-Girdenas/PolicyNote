 ...
 function [ std_Variables, std_Multipliers ] = steadyStateFunction(paramStruct)
    ...
    std_Variables = struct('Sst',Sst,'bst',bst,'qst',qst,'Cst',Cst,'Nst',Nst,'Kst',Kst,'Zst',Zst,'ust',ust);
        
    % Solve for steady state Lagrange multipliers
    
    syms G7s G8s G9s G1s G2s G3s G4s G5s
    
    %e1  = G7s*(paramStruct.epsilon-1)/paramStruct.epsilon*paramStruct.z + G7s*((paramStruct.epsilon-1)/paramStruct.epsilon)*(paramStruct.z/paramStruct.Xst)+G8s*paramStruct.z-paramStruct.a*G8s+paramStruct.beta*G8s*(1-paramStruct.lambda)*paramStruct.k*(qst^(-1/(1-paramStruct.alpha)))*(paramStruct.xi^(1/(1-paramStruct.alpha)))+G9s-paramStruct.beta*G9s*(1-paramStruct.lambda)+paramStruct.beta*G9s*(qst^(-paramStruct.alpha/(1-paramStruct.alpha)))*((paramStruct.xi^(paramStruct.alpha/(1-paramStruct.alpha)))*(1-paramStruct.lambda));
    ...
    %e3  = G1s*paramStruct.intrst*bst-G2s*paramStruct.intrst*bst+G3s*paramStruct.intrst*bst+G4s+G5s*(paramStruct.omega/(1-paramStruct.omega))*Kst+G7s*((paramStruct.epsilon-1)/paramStruct.epsilon)*paramStruct.omega*(paramStruct.epsilon-1)*Kst-G7s*paramStruct.omega*paramStruct.epsilon*Zst;
    %e4  = (Cst^(1-paramStruct.sigma))+G4s*paramStruct.sigma-G4s*paramStruct.sigma/paramStruct.beta - G8s*Cst;
    ...
    %e7  = (G1s*paramStruct.beta^2) - G1s + G2s - G3s;
    %e8  = -G1s*paramStruct.intrst*bst + G2s*paramStruct.intrst*bst - G3s*paramStruct.intrst*bst - G4s*paramStruct.beta - G7s*((paramStruct.epsilon-1)/(paramStruct.epsilon))*paramStruct.omega*paramStruct.beta*Kst + G7s*paramStruct.omega*paramStruct.beta*Zst;
